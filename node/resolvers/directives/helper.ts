import { isUserPartOfBuyerOrg } from '../queries/index'
...
export const validateAdminToken = async (
  ...
  if (hasAdminToken) {
    try {
      const authUser = await identity.validateToken(adminUserAuthToken)
...
export const validateApiToken = async (
  ...
  if (hasApiToken) {
    try {
      ...
      const authUser = await identity.validateToken(token)
